  ...
  return (
    <div className="h-screen overflow-hidden">
      ...
      <div className="flex h-full overflow-hidden">
        <DashboardSidebar />
        <div className="flex-1 h-full px-4 overflow-y-auto bg-white py-7">
          <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-3">
              <h2 className="text-4xl font-open-sans-semibold">Dashboard</h2>
              <div className="flex items-center gap-1">
                {location && location.map((loc: string) => (
                  <button key={loc} disabled={loc === active && true} onClick={() => handleNavigate(loc)} className={`${loc === active ? "text-primary/50" : "text-primary"}`}>{loc}</button>
                ))}
              </div>
            </div>
            {children}
