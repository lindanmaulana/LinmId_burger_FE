...
import { ServiceMidtransPayment, ServiceMidtransPaymentParams } from "../../../utils/services/midtrans-payment";
...
const PageMyOrder = () => {
  ...
  const {mutate} = useMutation({
    ...
    mutationFn: (data: ServiceMidtransPaymentParams) => ServiceMidtransPayment(data)
  })
            ...
            <table className="w-full">
              ...
              <tbody className="flex flex-col gap-4 text-center">
                {dataOrder.data.map((order: order, index: number) => (
                  <tr key={order._id} className="flex items-center justify-between bg-white rounded-lg shadow border-devWhitePurple">
                    <td className="p-2">{index + 1}</td>
                    <td className="p-2">{order._id}</td>
                    <td className="p-2">{order.total_price.toLocaleString("id")}</td>
                    <td className="p-2">
                        <p className={`${getStatusColorOrder(order.status)}`}>{order.status}</p>
                    </td>
                    <td className="p-2">
                      <button onClick={() => handlePayment({id_order: order._id, total_price: order.total_price})} className={`${order.status !== "paid" ? "bg-statusPaid" : "bg-statusCanceled"} rounded p-1 text-white text-sm`}>{loading ? "Loading.." : "Payment"}</button>
                    </td>
                  </tr>
                ))}
              </tbody>
